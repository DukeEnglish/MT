...
def initial_state():
  return state(k=-1,j=-1,i=0, lm_state=lm.begin())
...
def extend_state(s, f):
  for j in xrange(s.i+1,len(f)+1):
    ...
    # need to iterate through every possible adjacent phrase
        ...
        for k in range(s.i+1, j):
          
          if f[s.i:k] in tm and f[k:j] in tm:
            
            for phrase1 in tm[f[s.i:k]]:
              
              for phrase2 in tm[f[k:j]]:
                
                logprob2 =  phrase2.logprob

                # summate logprob for words in phrase 2
            lm_state2 = s.lm_state
            ...
            new_s2 = state(s.i,k,j, lm_state2)
            yield (new_s2, logprob2, phrase2)
            ...
            new_s1 = state(s.i,k,j, lm_state1)
            yield (new_s1, logprob1, phrase1)
